  ...
  useEffect(() => {
    ...
    itemRef.current.map((item, index) => {
      ...
      time.fromTo(
        ...
        {duration: 10,autoAlpha: 1,ease: "power2",clipPath: "polygon(0 0, 100% 0, 100% 100%, 0 100%)",}
      );
      time.fromTo(
    ...
    imageSrc.current.map((item, index) => {
      ...
      time.fromTo(
        ...
        {scale: 1,duration: 3,delay: 0,stagger: 0.3,autoAlpha: 1,ease: "power2",
        }
      );
      return time;
    });
    ...
    <div className='relative z-20 my-96'>
      ...
      {DataSection.map(({ id, src, subtitle, content }) => (
        <Section
          ...
          nodeRef={addRefs}
          className={`.image flex items-center gap-x-20 will-change-auto  ${
            id % 2 ? "flex-row" : "flex-row-reverse"
          }`}>
          <article className={`w-6/12 ${id % 2 ? "text-right" : "text-left"}`}>
            <div className='mb-3 '>
              <Subtitle name={subtitle} />
            </div>
            <div className={`w-7/12  ${id % 2 ? "ml-auto " : "mr-auto"}`}>
